...
ChannelGeometry::ChannelGeometry()
{
    ...
    // Initializes channel geometry and transfers it to VBOs
    setObjectGeometry(INITIAL_CHANNEL_LENGTH/SCALE, INITIAL_CHANNEL_WIDTH/SCALE, INITIAL_CHANNEL_HEIGHT/SCALE);
}
...
void ChannelGeometry::setObjectGeometry(qreal length, qreal width, qreal height)
{
    ...
    GLushort indices[] = {
        ...
        68, 68, 69, 70, 71      // Face 17

    };

    // Transfer vertex data to VBO 0
    arrayBuf.bind();
    arrayBuf.allocate(vertices, 72 * sizeof(VertexData));

    // Transfer index data to VBO 1
    indexBuf->bind();
    indexBuf->allocate(indices, 106 * sizeof(GLushort));
}

void ChannelGeometry::drawObjectGeometry(QOpenGLShaderProgram *program)
{
    ...
    glDrawElements(GL_TRIANGLE_STRIP, 106, GL_UNSIGNED_SHORT, 0);
}
